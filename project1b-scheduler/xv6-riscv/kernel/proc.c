...
uint64 sys_startlog(void){
  ...
  if(numLog != 0){
    //Logging has already started so return -1 error.
...
uint64 sys_getlog(void){
  uint64 userlog; //Hold the virtual user address of the user's copy of the log
  ...
  //TODO most of this function is done for you, just return the number of entires in the log.
  //^how many logs have been recorded since startlog was called
...
int sys_nice(void){
  ...
  struct proc *p = myproc();
  //Set over 19 to add 19, as well as under -20 to -20 and if not += inc to nice then return.
  if(inc > 19){
    p->nice += 19;
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        p->state = RUNNING;
        c->proc = p;
        //Fill up schedlog to size log size, we don't want same process ID in a row though
        if(numLog < LOG_SIZE && lastProcessId != p->pid){
          //Set schedlog pid
          schedlog[numLog].pid = p->pid;
          //Set last process ID so we don't have the same one multiple times in a row
          lastProcessId = p->pid;
          //Set the timer
          schedlog[numLog].time = timer;
          //Increment number of items in the log.
          numLog++;
        }
